...
from src.model.model import fit, vocabulary, fit_example
import json
...
def print_by_sentences(audio_file_path: str):
    sentences = load(audio_file_path)
    sentences = get_sentences(sentences)
    for sentence in sentences:
        input = ' '.join(word['text'] for part in sentence for word in part)
...
def cut_fillers_example(audio_file_path: str, ex: str, error=0.8):
    sentences = load(audio_file_path)
    sentences = get_sentences(sentences)
    start_all = []
    end_all = []
    clf, vectorizer = fit_example(ex)
